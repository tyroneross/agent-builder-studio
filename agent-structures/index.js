...
export const AGENT_STRUCTURES = [
  ...
  structure({
    id: "local-knowledge-agent",
    label: "Local Knowledge Agent",
    short: "Ingests files into local vectors, SQLite rows, and a small knowledge graph.",
    category: "operations",
    spec: {
      patternId: "solo-tool-agent",
      projectName: "Local Knowledge Agent",
      description:
        "Turns uploaded local files into parsed chunks, embeddings, SQL-searchable rows, semantic retrieval, a simple knowledge graph, and editable rich-text outputs.",
      ...
      modelProfiles: {
        hardwareTarget: "24GB Apple Silicon MacBook Pro with M4-class unified memory",
        runner: "Ollama for chat and embeddings; Omniparse for local document parsing when installed",
        contextPolicy: "Default to qwen3:14b with 32K effective context on a 24GB Mac. Chunk long files, retrieve only top prior chunks, and keep temperature low for source fidelity.",
        primary: {
          model: "qwen3:14b",
          install: "ollama run qwen3:14b",
          use: "Primary local knowledge synthesis model for a 24GB Mac.",
        },
        fallbacks: [
          {
            model: "gemma3:12b",
            install: "ollama run gemma3:12b",
            use: "Fallback for long-source summarization and concise note drafting.",
          },
          {
            model: "llama3.1:8b",
            install: "ollama run llama3.1:8b",
            use: "Fast fallback for short meetings, formatting passes, and low-latency review.",
          },
          {
            model: "nomic-embed-text",
            install: "ollama pull nomic-embed-text",
            use: "Default local embedding model for semantic retrieval.",
          },
        ],
        stretch: [
          {
            ...
            use: "Optional quality pass when the laptop has memory headroom and the transcript is already chunked.",
          },
          {
            model: "qwen3:30b",
            install: "ollama run qwen3:30b",
            use: "Optional deeper reasoning profile with smaller chunks and minimal concurrent apps.",
          },
        ],
      },
      learning: learningProfile({
        domain: "local document knowledge",
        metrics: ["parse coverage", "semantic retrieval quality", "SQL retrieval quality", "graph usefulness", "output editability"],
        skills: ["document parsing", "chunking", "local embeddings", "SQLite retrieval", "knowledge graph extraction", "rich-text output editing"],
        exemplars: ["multi-file project ingest", "customer call corpus", "board packet archive"],
      }),
      inputs: [
        "uploaded_documents",
        "retrieval_query",
      ...
      tools: [
        { name: "parse_documents_with_omniparse", responsibility: "Use local Omniparse for spreadsheets, PowerPoint, Python, PDF, and CSV/TSV documents, then fall back to internal text extraction when needed.", sideEffect: "read", permission: "allow-read" },
        { name: "chunk_and_embed_documents", responsibility: "Chunk parsed text and embed chunks through Ollama /api/embed or deterministic local hash vectors when Ollama is unavailable.", sideEffect: "write", permission: "ask-first" },
        { name: "write_local_database", responsibility: "Persist documents, chunks, entities, relations, graph nodes, graph edges, and run records in the local SQLite database.", sideEffect: "write", permission: "ask-first" },
        { name: "retrieve_local_knowledge", responsibility: "Search local knowledge by semantic vector similarity, SQL-backed text/entity lookup, or hybrid retrieval.", sideEffect: "read", permission: "allow-read" },
        { name: "verify_knowledge_outputs", responsibility: "Check source faithfulness, parser warnings, retrieval provenance, and requested output format before presenting rich text.", sideEffect: "none", permission: "allow-read" },
      ],
      nodes: [
        node("parse", "Document parser", "tool", "Parse uploaded .txt, .rtf, .pdf, spreadsheet, presentation, and related local files through Omniparse or internal fallback extraction.", 42, 132, { tools: ["parse_documents_with_omniparse"], inputs: ["uploaded_documents"], outputs: ["parsed_sources"] }),
        node("store", "Local database", "memory", "Chunk, embed, and persist file knowledge into JSON vectors and SQLite tables.", 286, 132, { tools: ["chunk_and_embed_documents", "write_local_database"], inputs: ["parsed_sources"], outputs: ["embedded_chunks", "sql_rows", "local_database"], permission: "ask-first" }),
        node("graph", "Knowledge graph", "memory", "Extract local entity nodes and co-occurrence relations from chunks.", 530, 68, { tools: ["write_local_database"], inputs: ["embedded_chunks"], outputs: ["knowledge_graph"], permission: "ask-first" }),
        node("retrieve", "Retrieval engine", "agent", "Run SQL, semantic, or hybrid retrieval with editable prompt and model controls.", 530, 252, { tools: ["retrieve_local_knowledge"], inputs: ["retrieval_query", "embedded_chunks", "sql_rows", "knowledge_graph", "model_controls"], outputs: ["semantic_results", "sql_results", "retrieved_context", "model_run_metrics"] }),
        node("format", "Rich text editor", "executor", "Render editable rich text that the user can revise after retrieval.", 774, 100, { inputs: ["retrieved_context", "prompt_guidance", "output_format_instructions"], outputs: ["rich_text_output"] }),
        node("verify", "Output verifier", "eval", "Verify source faithfulness, parser warnings, retrieval provenance, and requested output format.", 774, 286, { tools: ["verify_knowledge_outputs"], inputs: ["rich_text_output", "parsed_sources", "retrieved_context", "knowledge_graph"], outputs: ["quality_check"] }),
      ],
      edges: [
        { from: "parse", to: "store", label: "parsed sources" },
        { from: "store", to: "graph", label: "chunks" },
        { from: "store", to: "retrieve", label: "SQL + vectors" },
        { from: "graph", to: "retrieve", label: "entities" },
        { from: "retrieve", to: "format", label: "retrieved context" },
        { from: "format", to: "verify", label: "editable output" },
        { from: "graph", to: "verify", label: "graph provenance" },
      ],
      evals: [
        evalTask("local-db-written", "Ingest a mixed file bundle.", "Documents, chunks, entities, relations, and runs are persisted to local SQLite tables."),
        evalTask("hybrid-retrieval-visible", "Search the local corpus.", "The response separates semantic and SQL-backed retrieval evidence when hybrid mode is used."),
        evalTask("editable-format-honored", "Request a board-style output format.", "Rich-text output follows the requested section order."),
        evalTask("omniparse-fallback-visible", "Upload a mixed PDF and spreadsheet bundle.", "Parser source records show Omniparse when available and internal fallback warnings when needed."),
        evalTask("model-controls-recorded", "Change temperature and context settings.", "Model run metrics and selected controls are recorded with the output."),
      ],
      sources: ["omniparse-local-sdk", "ollama-api", "ollama-embed-api", "ollama-embedding-models", "ollama-qwen3", "ollama-gemma3", "ollama-llama3.1"],
    },
    sandbox: {
      prompt: "Upload a local project folder export and build SQL rows, semantic vectors, a knowledge graph, and editable rich-text retrieval output.",
      expectedArtifacts: ["local-database.sqlite", "vector-store.json", "knowledge-graph.json", "retrieved-context.json", "rich-text-output.html", "model-run-metrics.json"],
      requiredTerms: ["SQL", "semantic", "vector", "graph", "retrieval"],
      scenarios: [
        scenario(
          "multi-file-project",
          "Ingest a PDF, CSV, and notes file, then answer a project-risk query.",
          ["local-database.sqlite", "vector-store.json", "knowledge-graph.json", "rich-text-output.html"],
          ["SQL", "semantic", "graph", "source"],
          ["risk", "entity", "relation", "retrieval"],
          { files: ["brief.pdf", "tracker.csv", "notes.txt"] },
        ),
        scenario(
          "hybrid-search",
          "Search a local customer-call corpus with both SQL and semantic retrieval enabled.",
          ["retrieved-context.json", "knowledge-graph.json", "rich-text-output.html"],
          ["SQL", "semantic", "hybrid", "source"],
          ["customer", "objection", "entity"],
          { retrievalQuery: "pricing objection onboarding risk" },
        ),
        scenario(
          "mixed-doc-upload",
          "Process a PDF agenda, an XLSX issue tracker, and an RTF transcript into one local searchable corpus.",
          ["source-inventory.json", "local-database.sqlite", "knowledge-graph.json", "rich-text-output.html"],
          ["PDF", "XLSX", "RTF", "SQL"],
          ["Omniparse", "fallback", "source"],
          { parserMode: "auto" },
        ),
      ],
    },
