...
import { MAC_RAM_PROFILES, profileByRam } from "./model-profiles.mjs";

const DEFAULT_STORE_DIR = join("data", "store");
...
const DEFAULT_CHAT_MODEL = process.env.LOCAL_KNOWLEDGE_CHAT_MODEL ?? "qwen3:14b";
const DEFAULT_EMBEDDING_MODEL = process.env.LOCAL_KNOWLEDGE_EMBED_MODEL ?? "nomic-embed-text";
const DEFAULT_OMNIPARSE_ENTRY = process.env.OMNIPARSE_SDK_PATH ?? "";
...
export async function analyzeMeetingUploads(uploads, options = {}) {
  ...
  if (!files.length) throw new Error("At least one source file is required.");
  ...
  const meetingTitle = cleanInline(options.knowledgeTitle || options.title || options.meetingTitle) || titleFromFiles(readable);
  ...
  const documentId = `source-${transcriptHash.slice(0, 16)}`;
  ...
  return {
    ok: true,
    schemaVersion: "local-knowledge-agent.result.v1",
    documentId,
    meetingTitle,
    createdAt: now,
...
export async function initializeLocalStore(options = {}) {
  await mkdir(dirname(resolveStoreFile(options)), { recursive: true });
  await mkdir(dirname(resolveDbFile(options)), { recursive: true });
  const store = await loadMeetingStore(options);
  await saveMeetingStore(store, options);
  await saveKnowledgeDatabase(rebuildKnowledgeGraph(store), options);
  return storeStats(store, options);
}

export const ingestKnowledgeFiles = analyzeMeetingUploads;
export const searchKnowledge = searchMeetingMemory;
export const getStoreStats = getMeetingStoreStats;

export function buildMeetingInstallBundle(options = {}) {
  const profile = profileByRam(options.ramProfile ?? "24gb");
  const createdAt = options.createdAt ?? new Date().toISOString();
  return {
    schemaVersion: "local-knowledge-agent.install-manifest.v1",
    name: "Local Knowledge Agent",
    createdAt,
    selfContained: true,
    installType: "standalone-folder-package",
    ...
    install: {
      ...
      copyPolicy: "Install this folder as a unit. The JSON manifest describes the runtime, storage contract, and model defaults.",
    },
  };
}

export const buildInstallManifest = buildMeetingInstallBundle;
...
export function buildMeetingMarkdown({ meetingTitle, output, retrievedContext = [], files = [], createdAt }) {
  ...
  const context = retrievedContext.length
    ...
    : "- No prior local context retrieved.";

  return `---
schema: local-knowledge-agent.output.v1
...
async function summarizeWithOllama({ title, text, files, retrievedContext, guidance, chatModel, outputInstructions, modelOptions = {} }) {
  ...
  const prompt = {
    sourceTitle: title,
    operatorGuidance: cleanText(guidance || "Summarize for a human operator who needs notes, action items, next steps, decisions, and open questions."),
    ...
    sourceInventory,
    priorLocalContext: prior,
    sourceText: compactTranscriptForPrompt(text),
    requiredOutput: {
  ...
  const response = await fetch(`${DEFAULT_OLLAMA_BASE}/api/chat`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You produce source-faithful local knowledge notes. Return only valid JSON. Do not invent owners, due dates, decisions, or facts.",
        },
        {
          role: "user",
  ...
  try {
    ...
    throw new Error("ollama returned non-JSON local knowledge output");
  }
}
...
async function parseWithOmniparse({ name, buffer, extension, options }) {
  ...
  const tmpRoot = await mkdtemp(join(tmpdir(), "local-knowledge-omniparse-"));
...
function extractiveSummary({ title, text, files, retrievedContext }) {
  ...
  return {
    ...
    notes: noteSentences.map((item) => ({ text: item.text, source: "source" })),
    ...
    decisions: decisionLines.map((item) => ({ text: item.text, source: "source" })),
    openQuestions: questionLines.map((text) => ({ text, owner: ownerFromText(text) })),
  };
}

function normalizeSummaryOutput(value, fallbackText) {
  const fallback = extractiveSummary({ title: "Local Knowledge", text: fallbackText, files: [], retrievedContext: [] });
  return {
    ...
    actionItems: normalizeObjectList(value?.actionItems, "task").map((item) => ({
      ...
      task: cleanInline(item.task || item.text) || "Review follow-up.",
...
async function readUpload(upload) {
  const name = cleanInline(upload?.name) || "source-upload.txt";
...
function resolveStoreFile(options = {}) {
  ...
  const configured = options.storePath ?? process.env.LOCAL_KNOWLEDGE_VECTOR_STORE_PATH ?? DEFAULT_STORE_DIR;
...
function emptyStore() {
  ...
  return {
    schemaVersion: "local-knowledge-agent.store.v1",
    createdAt: now,
    updatedAt: now,
    documents: [],
    chunks: [],
...
function resolveDbFile(options = {}) {
  const root = resolve(options.root ?? process.cwd());
  const configured = options.dbPath ?? process.env.LOCAL_KNOWLEDGE_DB_PATH ?? join(DEFAULT_STORE_DIR, DEFAULT_DB_FILE);
...
function compactTranscriptForPrompt(text) {
  ...
  return [
    ...
    "\n\n[Middle source content compressed]\n\n",
    actionLines,
...
function titleFromFiles(files) {
  return cleanInline(files[0]?.name?.replace(/\.[^.]+$/, "")) || "Local Knowledge Source";
}
